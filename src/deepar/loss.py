...
def accuracy_ROU(rou: float, samples: torch.Tensor, labels: torch.Tensor, relative=False):
    ...
    for t in range(labels.shape[1]):
        ...
        #         2
        #         * (
        #             torch.sum(rou * abs_diff[labels[:, t][zero_index] > rou_pred])
        #             - torch.sum((1 - rou) * abs_diff[labels[:, t][zero_index] <= rou_pred])
        #         ).item()
        #     )
        ...
        numerator += (2 * (torch.sum(rou * abs_diff[labels[:, t][:] > rou_pred]
                                     ) - torch.sum((1 - rou) * abs_diff[labels[:, t][:] <= rou_pred])).item())

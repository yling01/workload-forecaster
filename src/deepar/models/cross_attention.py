...
class pBLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        ...
        self.dropout = LockedDropout(0.1)

    def forward(self, x):
        # X: (B, T, dim)
        x = self.dropout(x)

        # Chop off first input if T is odd
        if x.shape[1] % 2 == 1:
            x = x[:, 1:, :]

        # Concatenate neighboring 2 hiddens
        out_cat = x.reshape((x.shape[0], x.shape[1] // 2, x.shape[2] * 2))  # (B, T/2, dim*2)

        out_lstm, (hidden, c) = self.blstm(out_cat)  # (B, T/2, hidden_dim*2)
        return out_lstm
...
class Encoder(nn.Module):
    def __init__(self, params, num_classes):
        ...
        self.pBLSTMs = nn.ModuleList(
            [pBLSTM(params.lstm_hidden_dim * 4, params.lstm_hidden_dim) for _ in range(self.params.plstm_layers)]
        )
        ...
        # self.freq_transform = FrequencyMasking(3)
    ...
    def forward(self, zxs, series_ids):
        ...
        output, (hidden, cell) = self.lstm(lstm_input, (hidden, cell))  # (B, T, hidden*2)

        for pBLSTM in self.pBLSTMs:
            output = pBLSTM(output)  # (B, T, hidden*2)
...
class DeepARSeq2Seq(nn.Module):
    ...
    def forward(self, zxs, series_ids):
        ...
        Args:
            ...
            series_ids: (B, )
        """
        key, value, idx_embed = self.encoder(zxs, series_ids)

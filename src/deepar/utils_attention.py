...
def get_metrics(sample_mu, labels, predict_start, samples=None, relative=False):
    ...
    metric = dict()
    metric["ND"] = loss_attention.accuracy_ND_(sample_mu, labels[:, predict_start:], relative=relative)
    metric["RMSE"] = loss_attention.accuracy_RMSE_(sample_mu, labels[:, predict_start:], relative=relative)
    if samples is not None:
        metric["rou90"] = loss_attention.accuracy_ROU_(0.9, samples, labels[:, predict_start:], relative=relative)
        metric["rou50"] = loss_attention.accuracy_ROU_(0.5, samples, labels[:, predict_start:], relative=relative)
    return metric
...
def update_metrics(
    ...
    raw_metrics["ND"] = raw_metrics["ND"] + loss_attention.accuracy_ND(
        sample_mu, labels[:, predict_start:], relative=relative
    )
    raw_metrics["RMSE"] = raw_metrics["RMSE"] + loss_attention.accuracy_RMSE(
        sample_mu, labels[:, predict_start:], relative=relative
    )
    ...
    if input_mu is not None:
        ...
        raw_metrics["test_loss"] = raw_metrics["test_loss"] + [
            loss_attention.NLL_loss(input_mu, input_sigma, labels[:, :predict_start]),
            input_time_steps,
        ]
    ...
    raw_metrics["test_pred_loss"] = raw_metrics["test_pred_loss"] + [
        loss_attention.NLL_loss(sample_mu, sample_sigma, labels[:, predict_start:]) * sample_time_steps,
        sample_time_steps,
    ]

    if samples is not None:
        raw_metrics["rou90"] = raw_metrics["rou90"] + loss_attention.accuracy_ROU(
            0.9, samples, labels[:, predict_start:], relative=relative
        )
        raw_metrics["rou50"] = raw_metrics["rou50"] + loss_attention.accuracy_ROU(

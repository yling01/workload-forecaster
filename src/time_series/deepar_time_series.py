...
def train(model: nn.Module,
    ...
    # train_window is the seq_len
    for i, (train_batch, idx, labels_batch) in enumerate(tqdm(train_loader)):
        optimizer.zero_grad()
        batch_size = train_batch.shape[0]

        # train_batch -> (train_window, batch_size, 1+cov_dim)
        # Note how z_t is folded into train_batch with x_m
        # labels_batch -> (train_window, batch_size)

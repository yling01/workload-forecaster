...
OUTPUT_DIR = "./admission_results/online-clustering-results/"
...
def AdjustCluster(
    ...
    for t in sorted(data.keys()):
        ...
        if new_ass[t] != -1:
            ...
            print("%s: template %s quit from cluster %d with total %d" % (next_date, cnt, cluster, total_queries[t]))
        ...
        if new_cluster != None:
            if new_ass[t] == -1:
                print(
                    "%s: template %s joined cluster %d with total %d" % (next_date, cnt, new_cluster, total_queries[t])
                )
            else:
                print(
                    "%s: template %s reassigned to cluster %d with total %d"
                    % (next_date, cnt, new_cluster, total_queries[t])
                )
        ...
        if new_ass[t] == -1:
            print(
                "%s: template %s created cluster as %d with total %d"
                % (next_date, cnt, next_cluster, total_queries[t])
            )
        else:
            print(
                "%s: template %s recreated cluster as %d with total %d"
                % (next_date, cnt, next_cluster, total_queries[t])
            )
...
def OnlineClustering(min_date, max_date, data, total_queries, rho):
    ...
    for i in range(num_gaps):
        ...
        assign, next_cluster = AdjustCluster(
            month_min_date,
            current_date,
            next_date,
            data,
...
if __name__ == "__main__":
    ...
    aparser.add_argument(
        "--dir", default="combined-results", help="The directory that contains the time series" "csv files"
    )
    aparser.add_argument("--project", help="The name of the workload")
    aparser.add_argument(
        "--rho", default=0.8, help="The threshold to determine" "whether a query template belongs to a cluster"
    )
    ...
    num_clusters, assignment_dict, cluster_totals = OnlineClustering(
        min_date, max_date, data, total_queries, float(args["rho"])
    )

    with open(
        OUTPUT_DIR + "{}-{}-assignments.pickle".format(args["project"], args["rho"]), "wb"

...
DATA_DICT = {
    ...
    "admissions": "admission_results/admission-combined-csv/",
...
def GenerateData(
    min_date, max_date, data, data_accu, templates, assignment_dict, total_queries, num_clusters, output_csv_dir
):
    plotted_total = 0
    plotted_cnt = 0
    totals = []
    ...
    for current_date, assignments in assignment_dict:
        ...
        for template, cluster in assignments.items():
            ...
            last_total_date = next(data_accu[template].irange(maximum=current_date, reverse=True))
        ...
        for template, cluster in assignments.items():
            ...
            for d in data[template].irange(
                last_date + record_ahead_time, current_date + record_ahead_time, (True, False)
            ):
                if not d in online_clusters[cluster]:
                    online_clusters[cluster][d] = data[template][d]
                else:
...
def Main(project, assignment_path, output_csv_dir, output_dir):
    ...
    top_clusters, coverage = GenerateData(
        min_date, max_date, data, data_accu, templates, assignment_dict, total_queries, num_clusters, output_csv_dir
    )

...
def ProcessData(path, output_dir, num_logs, config):
    ...
    try:
        ...
        for query_info in reader:
            ...
            time_stamp = datetime.datetime.strptime(time_stamp, config["time_stamp_format"])
    ...
    MakeCSVFiles(
        templated_workload, min_timestamp, max_timestamp, output_dir + "/" + path.split("/")[-1].split(".gz")[0] + "/"
    )
...
def MakeCSVFiles(workload_dict, min_timestamp, max_timestamp, output_dir):
    ...
    for template in workload_dict:
        # print(template)
        template_timestamps = workload_dict[template]  # time stamps for ith cluster
        ...
        # write to csv file
        with open(output_dir + "template" + str(template_count) + ".csv", "w") as csvfile:
...
if __name__ == "__main__":
    ...
    aparser.add_argument(
        "--max_log",
        type=int,
        help="Maximum number of logs to process in a" "data file. Process the whole file if not provided",
    )
